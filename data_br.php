<?php
    ...
    $aColumns = array('protein', 'position', 'gene', 'distance', 'annotation', 'pvalue');
    $sIndexColumn = "protein";
    $sTable =  isset($_GET['pTb']) ?  $_GET['pTb'] : 'hg19_pdi_chip';
    ...
    while ( $aRow = mysql_fetch_array( $rResult ) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            {
                $row[] = '<a href="browser.php?location=' . $aRow['position'] . '">' . $aRow['position'] . '</a>';
            }
            else if ( $aColumns[$i] != ' ' )
            {
                $row[] = $aRow[$aColumns[$i]];
